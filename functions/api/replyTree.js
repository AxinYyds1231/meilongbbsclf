...
export async function onRequest(context) {
    ...
    if (request.method === 'GET') {
        ...
        if (!postId) return new Response(JSON.stringify({ error: '缺少帖子ID' }), { status: 400, headers: CORS_HEADERS });
        try {
    ...
    if (request.method === 'POST') {
        ...
        if (!session) return new Response(JSON.stringify({ error: '请先登录' }), { status: 401, headers: CORS_HEADERS });
        ...
        try {
            ...
            if (!postId || !content) return new Response(JSON.stringify({ error: '参数不完整' }), { status: 400, headers: CORS_HEADERS });
            if (content.length > 500) return new Response(JSON.stringify({ error: '回复不能超过500字' }), { status: 400, headers: CORS_HEADERS });
            ...
            if (!reply) return new Response(JSON.stringify({ error: '帖子不存在' }), { status: 404, headers: CORS_HEADERS });

            await db.incrementStats('reply');

            // @提及
            const mentioned = extractMentions(filtered);
            for (const targetUid of mentioned) {
                if (targetUid === uid) continue;
                const target = await db.findUserByUid(targetUid);
                if (target) {
                    await db.sendMessage('admin', targetUid, `${name} 在回复中提到了你`, 'system');
                }
            }
