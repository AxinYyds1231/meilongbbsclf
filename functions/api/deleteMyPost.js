...
export async function onRequest(context) {
    ...
    try {
        ...
        if (post.authorUid !== uid) {
            return new Response(JSON.stringify({ error: '无权删除此帖子' }), { status: 403, headers: CORS_HEADERS });
        }

        // 执行软删除（标记 deleted=true，记录删除原因）
        const success = await db.deletePostById(postId, '用户自行删除', uid);
        if (!success) {
            return new Response(JSON.stringify({ error: '删除失败' }), { status: 500, headers: CORS_HEADERS });
        }
